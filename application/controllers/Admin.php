...
class Admin extends CI_Controller
{
    ...
    public function index()
    {
        ...
        //mengupdate stok dan dibooking pada tabel buku
          $detail = $this->db->query("SELECT*FROM booking,booking_detail WHERE DAY(curdate()) < DAY(batas_ambil ) AND booking.id_booking=booking_detail.id_booking")->result_array();         foreach ($detail as $key) {             $id_buku = $key['id_buku'];             $batas = $key['tgl_booking'];             $tglawal = date_create($batas);             $tglskrg = date_create();             $beda = date_diff($tglawal, $tglskrg); 
        ...
        //menghapus otomatis data booking yang sudah lewat dari 2 hari         
            ...
            if (!empty($booking)) {             
                foreach ($booking as $bo) {                 $id_booking = $booking->id_booking;                 
